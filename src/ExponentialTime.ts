...
export class ExponentialTime {
    ...
    constructor(private _initialTime: number,
        ...
        invariant(this._initialTime > 0, 'Initial delay must be positive');
        invariant(this._maxTime > 0, 'Delay upper bound must be positive');
        invariant(this._growFactor >= 0, 'Ratio must be non-negative');
        invariant(this._jitterFactor >= 0, 'Jitter factor must be non-negative');
